...
bool load_media(void) 
{
    ...
    // Load splash image
    char * splash_image_path = "assets/lena.bmp";
    g_splash_image = SDL_LoadBMP(splash_image_path);
    if (g_splash_image == NULL) 
    {
        printf("Unable to load image %s! SDL Error: %s\n", splash_image_path, SDL_GetError());
        success = false;
    }
...
int main(int argc, char* args[]) {
    ...
    if (!init())
    {
        ...
        if (!load_media())
        {
			...
			//SDL_FillRect(screenSurface, NULL, SDL_MapRGB(screen_surface->format, 0xFF, 0xFF, 0xFF));
